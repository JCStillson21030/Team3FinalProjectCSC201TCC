...
int main()
{
    ...
    cout << "Here is Person's weight in descending order: " << endl << endl;
    ...
    for(int j = 0; j < pcount; j++)
    {
        for (int i = 1; i < pcount; i++)
        {
            ...
            {
                tmp3 = Persons[i].weight;
                Persons[i].weight = Persons[i - 1].weight;
                Persons[i - 1].weight = tmp3;
                
                tmp4 = Persons[i].pname;
                Persons[i].pname = Persons[i - 1].pname;
                Persons[i - 1].pname = tmp4;
            }
        }
    }
    people.close();
